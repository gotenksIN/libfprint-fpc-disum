...
/**
 * @brief Score how closely two prints' descriptor sets match
 *
 * @details Asymmetric: this runs a k-NN search, so @p frame supplies the
 * query descriptors and @p enrolled supplies the train descriptors being
 * searched -- swapping the two arguments changes the score. Despite the
 * parameter names, libfprint's fpi_print_sigfm_match() passes the
 * enrolled/template gallery entry as @p frame (the query set) and the
 * newly captured probe as @p enrolled (the train set).
 *
 * @param frame The k-NN query set (in libfprint's caller: the enrolled/
 *              template print)
 * @param enrolled The k-NN train set (in libfprint's caller: the newly
 *                 captured probe)
