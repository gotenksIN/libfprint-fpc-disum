                             ...
                             const guint8 *sig, gsize sig_len)
{
  ...
  fpc1022_sha256 ("FPC_HMAC_KEY", 13, hmac_key);
                         ...
                         guint8 *psk_out, gsize psk_out_size, gsize *psk_len_out)
{
  ...
  fpc1022_sha256 ("FPC_SEALING_KEY", 16, sealing_key);
                        ...
                        FpiUsbTransferCallback callback)
{
  ...
  transfer->ssm = ssm;
  fpi_usb_transfer_submit (transfer, FPC1022_CTRL_TIMEOUT, NULL,
                           callback, NULL);
}
...
static void fpc1022_process_event (FpDevice              *dev,
                                   FpiSsm                *ssm,
                      ...
                      gpointer user_data, GError *error)
{
  ...
  if (self->bulk_recv_len < sizeof (Fpc1022EvtHdr))
    {
      fpc1022_submit_bulk_read (dev, transfer->ssm, fpc1022_open_bulk_cb,
  ...
  fpc1022_process_event (dev, transfer->ssm, fpc1022_open_bulk_cb);
}
      ...
      if (pending_out > 0)
        {
          ...
          int n = BIO_read (self->bio_out, buf,
                            MIN (pending_out, FPC1022_BULK_MAX_PKT));
          if (n > 0)
            {
              ...
              fpi_usb_transfer_submit (t, FPC1022_CTRL_TIMEOUT, NULL,
                       ...
                       FpiUsbTransferCallback bulk_cb)
{
  ...
  switch (ssm_state)
    ...
    case FPC1022_OPEN_WAIT_INIT_RESULT:
      ...
      fpc1022_submit_bulk_read (dev, ssm, bulk_cb, FPC1022_DATA_TIMEOUT);
      return;
                             ...
                             gpointer user_data, GError *error)
{
  ...
  if (pkt->magic != FPC1022_TLS_KEY_MAGIC)
    {
      fp_err ("TLS key packet bad magic: 0x%08x (expected 0x%08x)",
              pkt->magic, FPC1022_TLS_KEY_MAGIC);
      fpi_ssm_mark_failed (transfer->ssm,
                           fpi_device_error_new (FP_DEVICE_ERROR_PROTO));
      return;
    }
  ...
  if (!fpc1022_verify_tls_key_hmac (data + pkt->aad_offset, pkt->aad_len,
  ...
  if (!fpc1022_decrypt_tls_psk (data + pkt->key_offset, pkt->key_len,
...
fpc1022_open_ssm_run (FpiSsm *ssm, FpDevice *dev)
{
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case FPC1022_OPEN_PARSE_TLS_KEY:
                         ...
                         gpointer user_data, GError *error)
{
  ...
  gboolean waiting_finger = (ssm_state == FPC1022_CAPTURE_WAIT_FINGER ||
                             ssm_state == FPC1022_CAPTURE_ARM_SENSOR);
  guint timeout = waiting_finger ? FPC1022_FINGER_TIMEOUT : FPC1022_DATA_TIMEOUT;
  ...
  if (self->bulk_recv_len < sizeof (Fpc1022EvtHdr))
    {
      fpc1022_submit_bulk_read (dev, transfer->ssm, fpc1022_capture_bulk_cb, timeout);
      return;
    }
  ...
  fpc1022_capture_process_event (dev, transfer->ssm);
}
...
fpc1022_capture_process_event (FpDevice *dev, FpiSsm *ssm)
{
  ...
  gboolean waiting_finger = (state == FPC1022_CAPTURE_WAIT_FINGER ||
                             state == FPC1022_CAPTURE_ARM_SENSOR);
  guint timeout = waiting_finger ? FPC1022_FINGER_TIMEOUT : FPC1022_DATA_TIMEOUT;
  ...
  if (self->tls_in_image)
    {
      ...
      if (self->img_recv_len >= self->img_expected_len)
        {
          ...
          g_autoptr(FpImage) img = fp_image_new (FPC1022_IMG_WIDTH,
                                                 FPC1022_IMG_HEIGHT);
          memcpy (img->data, self->img_buf,
                  MIN (self->img_expected_len, (gsize) FPC1022_IMG_PIXELS));
          img->flags = 0;

          FpImage *scaled = fpc1022_scale_nn_2x (img);
      ...
      fpc1022_flush_tls_output (dev);
      fpc1022_submit_bulk_read (dev, ssm, fpc1022_capture_bulk_cb,
                                FPC1022_DATA_TIMEOUT);
      return;
    }
    ...
    case FPC1022_EVT_IMAGE:
      {
        ...
        gsize img_payload = (total_len > FPC1022_TLS_MSG_HDR_SIZE) ?
                            total_len - FPC1022_TLS_MSG_HDR_SIZE : 0;
        fp_dbg ("Image (via TLS), total_len=%u, payload=%lu bytes",
                total_len, (unsigned long) img_payload);

        self->tls_in_image = TRUE;
        self->img_recv_len = 0;
        self->img_expected_len = MIN (img_payload, (gsize) FPC1022_IMG_SIZE);

        /* Image pixels start after the 24-byte TLS message header */
        gsize img_data_off = FPC1022_TLS_MSG_HDR_SIZE;
...
fpc1022_capture_ssm_run (FpiSsm *ssm, FpDevice *dev)
{
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case FPC1022_CAPTURE_STOP_ABORT:
      ...
      fpc1022_send_ctrl (dev, ssm, FPC1022_CMD_ABORT, 0, 0, NULL, 0);
      break;
    ...
    case FPC1022_CAPTURE_WAIT_FINGER:
...
fpc1022_deact_ssm_run (FpiSsm *ssm, FpDevice *dev)
{
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case FPC1022_DEACT_ABORT:
      fp_dbg ("Sending abort");
      fpc1022_send_ctrl_full (dev, ssm, FPC1022_CMD_ABORT, 0, 0, NULL, 0,
                              fpc1022_ctrl_cmd_ignore_error_cb);
      break;
...
fpc1022_img_open (FpImageDevice *dev)
{
  ...
  self->img_buf = g_malloc0 (FPC1022_IMG_SIZE);
  self->img_recv_len = 0;
