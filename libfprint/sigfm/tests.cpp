...
TEST_CASE("self-match outscores cross-match and matcher terminates")
{
    constexpr auto img_w = 256;
    constexpr auto img_h = 256;
    constexpr auto img = embedded::capture_aes3500;

    SigfmImgInfo* a = sigfm_extract(img, img_w, img_h);
    REQUIRE(a != nullptr);

    /* No second embedded fingerprint exists in this file, so build a
     * deterministic pseudo-random pattern that carries no real
     * correspondence to a fingerprint image, to stand in as the
     * "distinct print" cross-match target. */
    std::vector<SigfmPix> unrelated(static_cast<std::size_t>(img_w) * img_h);
    for (std::size_t i = 0; i < unrelated.size(); i++)
        unrelated[i] = static_cast<SigfmPix>((i * 2654435761u) >> 24);
    SigfmImgInfo* b = sigfm_extract(unrelated.data(), img_w, img_h);
    REQUIRE(b != nullptr);

    const int self = sigfm_match_score(a, a);
    const int cross = sigfm_match_score(a, b);
    CHECK(self > 0);
    CHECK(self > cross);
    CHECK(cross >= 0);
